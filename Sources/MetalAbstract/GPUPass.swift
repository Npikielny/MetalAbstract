...
open class GPUPass {
    ...
    public init(pass: Pass, completion: @escaping (_ gpu: GPU) async throws -> Void) {
        self.pass = pass
        self.completion = completion
    }
    public init(@GPUPassBuilder pass: PassBuilder, completion: @escaping (_ gpu: GPU) async throws -> Void = { _ in }) rethrows {
        self.pass = try pass()
        self.completion = completion
    }
}
